...
public class IplStatsController {
    ...
    @GetMapping("/export-players/all")
    public ResponseEntity<byte[]> exportAllPlayers() {
        File file = iplStatsService.exportAllPlayers();
        return exportFile(file);
    }

    @GetMapping("/export-pdf-players/{team}")
    public ResponseEntity<byte[]> exportPlayersPdf(@PathVariable("team") String team) {
        File file = iplStatsService.exportPdfPlayers(team);
        return exportFile(file);
    }

    @GetMapping("/export-pdf-players/all")
    public ResponseEntity<byte[]> exportAllPlayersPdf() {
        File file = iplStatsService.exportPdfAllPlayers();
        return exportFile(file);
    }

    private ResponseEntity<byte[]> exportFile(File file) {
